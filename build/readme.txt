...
While the advantages of this are incredible, you no longer have an obvious way to take *any* type of object. If you want to accomplish this, you could use the undocumented "interface" keyword. An interface is a class that has no code, and as a result cannot be created directly. Say CTest::function() can take any object, so long as it has a run() method; you might code the following:
