...
Sub loadmain()
'mainForm File Filename
    'autorun mainForm.gam
    On Error Resume Next
    ...
    Dim ex As String
    Dim tPrgName As String
    If Command$ <> "" Then
        ex$ = GetExt(GetSpacedElement(Command$, 1))
        If UCase$(ex$) = "GAM" Then
            'call tracestring("Loading " + gampath$ + Command$)
            
            Call openMainFile(gamPath$ + Command$)
            Call setupmain
            Exit Sub
        Else
            'assume program...
            tPrgName$ = GetSpacedElement(Command$, 1)
            projectPath$ = GetSpacedElement(Command$, 2)
            'Call loadFont(projectPath$ + fontpath$ + "base.fnt")
            Call runProgram(projectPath$ + prgPath$ + tPrgName$)
            End
        End If
    End If
    Dim aa As Long
    If FileExists(gamPath$ + "main.gam") Then
        'mainForm.gam exists.
        Call openMainFile(gamPath$ + "main.gam")
        Call setupmain
        Exit Sub
    End If
    ChDir (currentdir$)
    ...
    dlg.strDefaultFolder = gamPath$
    dlg.strSelectedFile = ""
    dlg.strTitle = "Open mainForm File"
    dlg.strDefaultExt = "gam"
    dlg.strFileTypes = "Supported Files|*.gam;*.tpk|RPG Toolkit mainForm File (*.gam)|*.gam|RPG Toolkit PakFile (*.tpk)|*.tpk|All files(*.*)|*.*"
    If Not (OpenFileDialog(dlg)) Then 'user pressed cancel
        'uncomment!!!!!!!!!!!!!!!
        gGameState = GS_QUIT
        End
    ...
    'currentdir$ = CurDir$
    ChDir (currentdir$)
    If loadedMainFile$ = "" Then Exit Sub: End
    Dim whichType As String
    whichType$ = GetExt(loadedMainFile$)
    If UCase$(whichType$) = "TPK" Then      'Yipes! we've selected a pakfile!
        'setup the pakfile system
