        ...
        If UCase$(lit1$) = "TARGET" Then
            If targetType = 0 Then
                ...
                If (isFightInProgress()) Then
                    Call addPlayerHP(num2, playerMem(theOne))
                    Call fightInformRemoveStat(PLAYER_PARTY, theOne, -num2, False)
                Else
                    aa = getVariable(playerMem(theOne).healthVar$, lit$, curhp, theProgram)
                    curhp = curhp + num2
            ...
            If targetType = 2 Then
                ...
                Call fightInformRemoveStat(ENEMY_PARTY, theOne, -num2, False)
    ...
    Else
        ...
        If UCase$(lit1$) = "TARGET" Then
            If targetType = 0 Then
                theOne = target
                If (isFightInProgress()) Then
                    Call addPlayerSMP(num2, playerMem(theOne))
                    Call fightInformRemoveStat(PLAYER_PARTY, theOne, -num2, True)
                Else
                    aa = getVariable(playerMem(theOne).smVar$, lit$, curhp, theProgram)
                    curhp = curhp + num2
            ...
            If targetType = 2 Then
                ...
                Call fightInformRemoveStat(ENEMY_PARTY, theOne, -num2, True)
...
Public Sub attackAllRPG(ByRef text As String, ByRef theProgram As RPGCodeProgram)

    Dim paras() As parameters, count As Long
    paras = getParameters(text, theProgram, count)
    If (count <> 1) Then
        Call debugger("AttackAll() requires one parameter-- " & text)
        Exit Sub
    End If
    If (paras(0).dataType <> DT_NUM) Then
        Call debugger("AttackAll() requires a numerical parameter-- " & text)
        Exit Sub
    End If
    ...
    If (targetType = TYPE_PLAYER) Then
        ...
        For i = 0 To 4
            If (LenB(playerListAr(i))) Then
                Call addPlayerHP(-paras(0).num, playerMem(i))
                Call fightInformRemoveStat(PLAYER_PARTY, i, paras(0).num, False)
            End If
        Next i
    ElseIf (targetType = TYPE_ENEMY) Then
        ' Enemies targeted
        For i = 0 To numEne - 1
            If (enemyMem(i).eneHP > 0) Then
                Call addEnemyHP(-paras(0).num, enemyMem(i))
                Call fightInformRemoveStat(ENEMY_PARTY, i, paras(0).num, False)
            End If
        Next i
    End If
