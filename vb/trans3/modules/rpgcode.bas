...
Sub DestroyPlayerRPG(text$, ByRef theProgram As RPGCodeProgram)
    ...
    Else
        ...
        Select Case UCase$(ext$)
            ...
            Case "TEM":
                num = FreeFile
                file$ = FindFile(projectPath$ + temPath$ + lit$)
                num = FreeFile
                file$ = FindFile(projectPath$ + temPath$ + lit$)
                'file$ = PakLocate(file$)
...
Sub GiveItemRPG(text$, ByRef theProgram As RPGCodeProgram)
    ...
    Else
        ...
        'theOne = -1
        'For t = 0 To 500
        '    If UCase$(inventory$(t)) = UCase$(lit$) Then theOne = t
        'Next t
        'If theOne <> -1 Then
...
Sub IncludeRPG(text$, ByRef theProgram As RPGCodeProgram)
    ...
    Else
        ...
        If Not PakFileRunning Then
            filen = FindFile(projectPath + prgPath + lit)
        Else
...
Sub MainFileRPG(text$, ByRef theProgram As RPGCodeProgram)
    ...
    Else
        ...
        For num = 0 To 11
            ...
            Next t
        Next num
        Call openMainFile(lit$)
        Call setupmain
        'TBD: run startup program
    End If
...
Sub Mp3PauseRPG(text$, ByRef theProgram As RPGCodeProgram)
    ...
    Else
        Dim ext As String
        ext$ = GetExt(lit$)
        dataUse$ = FindFile(projectPath$ + mediaPath$ + lit$)
...
Function MWinPrepare(text$) As String
    'Prepare text for the Mwin.
...
Sub MWinRPG(text$, ByRef theProgram As RPGCodeProgram)
    '#Mwin("Text for mwin <variable$>")
    'Put text in Mwin.
    On Error GoTo errorhandler
    Dim use As String, dataUse As String, number As Long, useIt As String, useIt1 As String, useIt2 As String, useIt3 As String, lit As String, num As Double, a As Long, lit1 As String, lit2 As String, lit3 As String, num1 As Double, num2 As Double, num3 As Double
    use$ = text$
    ...
    num = CountData(dataUse$)        'how many data elements are there?
    If num <> 1 Then
        Call debugger("Warning: MWin has more than 1 data element!-- " + text$)
    End If
    useIt$ = GetElement(dataUse$, 1)
    If useIt$ = "" Then
        Call debugger("Error: MWin has no data element!-- " + text$)
        Exit Sub
    End If
    a = GetValue(useIt$, lit$, num, theProgram)
    If a = 0 Then
        lit$ = str$(num)
    End If
    'Before putting this in the Mwin, let's splice it up!
    lit$ = MWinPrepare(lit$)
    Call AddToMsgBox(lit$, theProgram)
    ...
    Else
        ...
        Select Case UCase$(ee$)
            Case "GPH":
                ...
                tbmName$ = replaceChar(RemovePath(lit$), ".", "_") + "_newplyr" + ".tbm"
                Call SaveTileBitmap(projectPath$ + bmpPath$ + tbmName$, tbm)
                anm.animFrame(0) = tbmName$
                anmName$ = replaceChar(RemovePath(file$), ".", "_") + "_newplyr" + ".anm"
...
Sub PutRPG(text$, ByRef theProgram As RPGCodeProgram)
    ...
    Else
        ...
        file$ = FindFile(projectPath$ + tilePath$ + file$)
...
Sub VariableManip(text$, ByRef theProgram As RPGCodeProgram, _
 Optional ByVal noErrors As Boolean = False)
...
On Error Resume Next
    ...
    Select Case dType
        Case 0:
            ...
            '! MODIFED BY KSNiloc...
            For t = 2 To number - 1
                fType$ = MathFunction(textTst$, t)
                'MsgBox ftype$
                'MsgBox numberuse(2)
                Select Case fType$
                    Case ""
                        numberuse(t + 1) = numberuse(t)
                    Case "+"
                        numberuse(t + 1) = numberuse(t) + numberuse(t + 1)
                    Case "-"
                        numberuse(t + 1) = numberuse(t) - numberuse(t + 1)
                    Case "*"
                        numberuse(t + 1) = numberuse(t) * numberuse(t + 1)
...
Function Fbranch(text$, ByRef theProgram As RPGCodeProgram) As Long
    '#Fbranch (:label name)
    ...
    debugger "FBranch has deprecated into Branch()-- " & text
    Branch text, theProgram
    ...
    'number = CountData(dataUse$)        'how many data elements are there?
    'If number <> 1 Then
    '    Call debugger("Warning: FBranch has more than 1 data element!-- " + text$)
    'End If
    'useIt$ = GetElement(dataUse$, 1)
    'If useIt$ = "" Then
    '    Call debugger("Error: FBranch has no label element!-- " + text$)
    '    Exit Function
    'End If
    'Now to find where this is at:
    'Dim foundIt As Long, t As Long, test As String, oldPos As Long, prPos As Long
    'foundIt = -1
    'For t = 0 To theprogram.length
    '    test$ = theprogram.program$(t)
    '    If UCase$(test$) = UCase$(useIt$) Then foundIt = t: t = theprogram.length
    'Next t
    'If foundIt = -1 Then
    '    Call debugger("Error: FBranch label not found!-- " + text$)
    'Else
    '    oldPos = theprogram.programPos + 1
    '    theprogram.programPos = foundIt + 1
    '    'MsgBox Str$(oldpos) + Str$(programpos(prgnum))
    '    Dim retval As RPGCODE_RETURN
    '    Do While theprogram.programPos <> -1 And theprogram.programPos <> -2
    '        prPos = theprogram.programPos
    ...
    '        Exit Function
    '    End If
    '    If theprogram.programPos = -2 Then
    '        Fbranch = -2
    '    End If
    'End If
...
Sub WinRPG(text$, ByRef theProgram As RPGCodeProgram)
    '#Win()
    'Win game and reset.
    'On Error GoTo errorhandler
    
    '=========================================================================
    'Obsolete June 25, 2004 by KSNiloc
    '=========================================================================
    
    debugger "Win() is obsolete-- " & text
    
    'Dim oldSize As Long, oldCol As Long, aa As String
    'oldSize = fontSize
    'oldCol = fontColor
    'fontSize = 25
    'fontColor = vbQBColor(15)
    'Call Fade("#Fade(1)", theprogram)    'Fadeout
    'Call TextRPG("#Text(1, 1, " + chr$(34) + "        -You Win!-" + chr$(34), theprogram)
    'fontSize = oldSize
    'fontColor = oldCol
    'aa$ = WaitForKey()
...
Function addext(text$, ext$) As String
    On Error GoTo errorhandler
...
Sub AddToMsgBox(text$, ByRef theProgram As RPGCodeProgram)
    'Adds selected text to message window at line lineNum
    'Should resize mwin.
    On Error Resume Next
    'MsgBox (Str$(screenHeight) + " " + Str$(screenWidth) + " " + Str$(screenHeight * 1 / 0.75))
    'size mwin:
    ...
    If lineNum = 1 Then
        ...
        Else
            pPic$ = FindFile(projectPath$ + bmpPath$ + MWinPic$)
            If PakFileRunning Then
...
Public Sub spliceVariables( _
    ...
    retVal.dataType = DT_LIT
    retVal.lit = MWinPrepare(paras(0).lit)
...
Public Sub MBoxRPG( _
    ...
    Dim num As Double
    Select Case cd
        Case 1: num = MBox(MWinPrepare(paras(0).lit))
        Case 2: num = MBox(MWinPrepare(paras(0).lit), paras(1).lit)
        Case 3: num = MBox(MWinPrepare(paras(0).lit), paras(1).lit, paras(2).num)
        Case 4: num = MBox(MWinPrepare(paras(0).lit), paras(1).lit, paras(2).num, _
            paras(3).num)
        Case 5: num = MBox(MWinPrepare(paras(0).lit), paras(1).lit, paras(2).num, _
