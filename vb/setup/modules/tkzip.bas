    ...
    If recurseYN Then
        For t = 0 To subdirpos
            If subdirlist$(t) <> "" Then
                c$ = CurDir$
                ChDir (subdirlist$(t))
                Call AddDirRecurse(exclusion$, subdirlist$(t) + "\", recurseYN, totalfiles, filesadded)
                ChDir (c$)
            End If
        Next t
    End If
...
Sub ExtractDir(zipfile$, extractInto$)
    ...
    For t = 0 To cnt - 1
        ...
        'register ocx files
        If UCase$(GetExt(GetZipFilename(t))) = "OCX" Then
            Call ExecCmd("regsvr32 /s " + Chr$(34) + extractInto$ + GetZipFilename(t) + Chr$(34))
        End If
